var jsPsychEditedVirtualChinrest = (function (jspsych) {
  'use strict';

  const info = {
      name: "edited-virtual-chinrest",
      parameters: {
          ...
          //edit - added properties for re-estimation of distance
          /** the maximal distance allowed to the screen. */
          max_distance_allowed_cm: {
              type: jspsych.ParameterType.INT,
              pretty_name: "Maximal viewing distance (cm)",
              default: 100,
          },
          /** the maximal distance allowed to the screen. */
          min_distance_allowed_cm: {
            type: jspsych.ParameterType.INT,
            pretty_name: "Minimal viewing distance (cm)",
            default: 30,
          },
          //edit - end
      },
  ...
  /**
   * Edited version of **virtual-chinrest**, the only change from the original JS plugin is commented with '//edit'
   *
   ...
   */
  class EditedVirtualChinrestPlugin {
      constructor(jsPsych) {
      ...
      `;
          ...
          var min_dist_inc =  Math.round(trial.min_distance_allowed_cm / 2.54)
          var max_dist_inc =  Math.round(trial.max_distance_allowed_cm / 2.54)
          var blindspot_prompt_edit = `
          <p>Now we will quickly measure how far away you are sitting.</p>
          <p>Please make sure you sit about an arm's length distance from the screen.</p>
          <p>Distance estimation will repeat if distance is outside the range of ${trial.min_distance_allowed_cm} and ${trial.max_distance_allowed_cm}cm (${min_dist_inc} - ${max_dist_inc}in).</p>
          <div style="text-align: left">
            <ol>
              <li>Put your left hand on the <b>space bar</b>.</li>
              <li>Cover your right eye with your right hand.</li>
      ...
      `;
          ...
          const startBlindSpotPhase = () => {
              ...
              //edit - redo resets distance estimation process
              this.reps_remaining = trial.blindspot_reps;
              //edit - end
          ...
          const finishBlindSpotPhase = () => {
              ...
              trial_data["failed_attempts"] = this.failed_chinrest;
        ...
        `;
          ...
          function computeTransformation() {
              ...
              if (px2unit_scr > 0) {
                  ...
                  trial_data.scale_factor
				  //edit - removed automatic scaling, will be used later only when presenting stimuli
      ...
      `;
          ...
          const recordPosition = () => {
              ...
              if (this.reps_remaining <= 0) {
                  //edit - we want to make sure distance is within configured bounds,
                  // so we first calculate the distance if not within bounds re-run the distance est. procedure
                  const angle = 13.5;
                  // calculate average ball position
                  const sum = blindspot_config_data["ball_pos"].reduce((a, b) => a + b, 0);
                  const ballPosLen = blindspot_config_data["ball_pos"].length;
                  ...
                  trial_data['']
                  // calculate viewing distance in mm
                  const viewDistance_cm = ball_sqr_distance / Math.tan(deg_to_radians(angle)) / 10;
                  if (viewDistance_cm > trial.max_distance_allowed_cm | viewDistance_cm < trial.min_distance_allowed_cm) {
                    this.failed_chinrest = this.failed_chinrest + 1;
                    let change_pos_msg = 'move away from the screen and re-run the procedure by pressing space twice';
                    let viewDistance_cm_rounded = Math.round(viewDistance_cm * 100) / 100
                    if (viewDistance_cm > trial.max_distance_allowed_cm) {change_pos_msg = 'move closer to the screen and re-run the procedure by pressing space twice'}
                    const msg = `The estimated distance is ${viewDistance_cm_rounded}cm, and hence not within the expected bounds of [${trial.min_distance_allowed_cm} - ${trial.max_distance_allowed_cm}cm] ${change_pos_msg}` 
                    blindspot_config_data.ball_pos = [];
                    blindspot_config_data.slider_clck =  false;
                    alert(msg);
                    
                    this.reps_remaining = trial.blindspot_reps;
                    document.querySelector("#click").textContent = this.reps_remaining.toString();
                    resetAndWaitForBallStart();
                  }
                  else {
  ...
  EditedVirtualChinrestPlugin.info = info;

  return EditedVirtualChinrestPlugin;
